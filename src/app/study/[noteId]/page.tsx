...
import { Flashcard, getFlashcardsForNote, getFlashcardsByType } from "@/lib/notes/flashcards";
import { supabase } from "@/lib/supabaseClient";
import ImageModal from "@/components/notes/ImageModal";
import MultipleChoiceStudyCard from "@/components/notes/MultipleChoiceStudyCard";
  ...
  const loadFlashcards = async () => {
    try {
      ...
      if (studyMode === 'traditional') {
        const { traditional } = await getFlashcardsByType(noteId);
        cards = traditional;
      } else if (studyMode === 'multiple_choice') {
        const { multipleChoice } = await getFlashcardsByType(noteId);
        cards = multipleChoice;
      } else if (studyMode === 'mixed') {
        const { traditional, multipleChoice } = await getFlashcardsByType(noteId);
        cards = [...traditional, ...multipleChoice].sort(() => Math.random() - 0.5);
      } else {
        // Fallback a todas las flashcards
        cards = await getFlashcardsForNote(noteId);
      }
    ...
    <div className="min-h-screen bg-gray-900 flex flex-col">
      ...
      {currentCard.type === 'multiple_choice' ? (
        <MultipleChoiceStudyCard
          ...
          onAnswer={(isCorrect) => handleAnswer(isCorrect ? 'correct' : 'incorrect')}
          onNext={() => {
            if (currentIndex < flashcards.length - 1) {
              setCurrentIndex(prev => prev + 1);
            } else {
